...
void *HiddenNeuroneTask ( void *ptr ) {
	...
	while (1) {
		...
		pthread_spin_lock(&(Work->spinlock_CalculeNet));

	    if (Fini) {
			pthread_spin_unlock(&(Work->spinlock_CalculeNet));
	    	break;
	    }
		...
		pthread_spin_unlock(&(Work->spinlock_CalculeFini));

		pthread_spin_lock(&(Work->spinlock_TrainNet));
		...
		pthread_spin_unlock(&(Work->spinlock_TrainFini));
	}
...
int CreateWorkLoad (Network  *Net, TaskWork **TaskWorkLoad) {
	...
	*TaskWorkLoad = (TaskWork *) calloc(numCPU, sizeof(TaskWork));
	...
	for (i = 0; i < numCPU; i++) {
        ...
        pthread_spin_init(&((*TaskWorkLoad)[i].spinlock_CalculeNet), 0);
        pthread_spin_init(&((*TaskWorkLoad)[i].spinlock_TrainNet), 0);
        pthread_spin_init(&((*TaskWorkLoad)[i].spinlock_CalculeFini), 0);
        pthread_spin_init(&((*TaskWorkLoad)[i].spinlock_TrainFini), 0);
        pthread_spin_lock(&((*TaskWorkLoad)[i].spinlock_CalculeNet));
        pthread_spin_lock(&((*TaskWorkLoad)[i].spinlock_TrainNet));
        pthread_spin_lock(&((*TaskWorkLoad)[i].spinlock_CalculeFini));
        pthread_spin_lock(&((*TaskWorkLoad)[i].spinlock_TrainFini));
	}
...
GET_TSC(t1);
		...
		for (m = 0; m < numLearnFiles; m++) {
			...
			//////////////////////////////////////////////////////////////////////////////////////
			for (n = 0; n < numTask; n++)
				pthread_spin_unlock(&(TaskWorkLoad[n].spinlock_CalculeNet));

			for (n = 0; n < numTask; n++)
				pthread_spin_lock(&(TaskWorkLoad[n].spinlock_CalculeFini));
			...
			*(Neurone->LocErrorValp) = NetErreur*(1.0-Reponse*Reponse);	// assigne l'erreur du réseau à cet neurone
			...
			for (n = 0; n < numTask; n++)
				pthread_spin_unlock(&(TaskWorkLoad[n].spinlock_TrainNet));

			for (n = 0; n < numTask; n++)
				pthread_spin_lock(&(TaskWorkLoad[n].spinlock_TrainFini));
			/////////////////////////////////////////////////////////////////////////////////////
...
GET_TSC(t2);
	...
	for (n = 0; n < numTask; n++) {
	    ...
	    pthread_spin_destroy(&(TaskWorkLoad[n].spinlock_CalculeNet));
	    pthread_spin_destroy(&(TaskWorkLoad[n].spinlock_TrainNet));
	    pthread_spin_destroy(&(TaskWorkLoad[n].spinlock_CalculeFini));
	    pthread_spin_destroy(&(TaskWorkLoad[n].spinlock_TrainFini));
	}
	free(TaskWorkLoad);
	free(Thread);
